...
int main()
{
    ...
    const char* filename = "filename.bin";
    const int N = 10;
    Student students_read[N]; // массив для чтения данных
    FILE* file = fopen(filename, "rb");
    if (file) {
        fread(students_read, sizeof(Student), N, file);
        fclose(file);
    }
    ...
    /*
    // заполнение массива структур по вашему желанию
    FILE* file2 = fopen(filename, "wb");
    if (file) {
        fwrite(student, sizeof(Student), N, file2);
        fclose(file2);
    }*/
